...
- 插件固定使用 Companion runtime，不再保留插件侧 Python ASR fallback。
...
- 替换 Obsidian 插件 UI。
- 恢复插件直接启动 ASR service 的旧 fallback。
- 发言人分离。
- token 级流式 ASR。
- 不依赖虚拟声卡的系统音频捕获。
...
冻结的 v1 schema 以 [API contract](./API_CONTRACT.md#5-companion-discovery-契约) 和 [JSON Schema](./contracts/companion-discovery.schema.json) 为准。Companion 写入的 `companion.json` 必须符合以下字段约定：
...
- discovery 文件存在、schema 有效、`status` 为 `running`、`updatedAt` 未超过 30 秒且 `/health` 二次确认成功时，插件可使用 Companion 提供的 `baseUrl`。
- discovery 缺失、服务未运行、服务不可达、文件 stale 或文件无效时，插件展示明确错误。
- 如果 `updatedAt` 超过 30 秒，直接视为 stale；即使旧 `baseUrl` 当前 `/health` 成功，插件也不得把该文件视为 Companion 可用。
...
ASR runtime 固定使用 Companion。插件只读取 Companion discovery，并调用 Companion 管理的 ASR endpoint；不再提供 `Auto`/`Manual` 后端选择，也不再从插件侧启动 Python ASR service。

持久化值固定为 `companion`。完整 discovery 和错误码映射以 [API contract](./API_CONTRACT.md#6-asr-runtime-契约) 为准。
...
插件只使用 Companion 管理的 ASR service，不再直接启动 Python ASR 进程。启动、停止和重启服务都在 Companion app 中完成。
...
- 插件固定使用 Companion discovery。
- Companion 不可用时插件展示明确错误。
- 插件侧 v0.1.0 ASR 启动流程不再可用。
...
2. Obsidian 插件读取 Companion discovery。
3. 开始一次会议。
4. 确认 Transcript 收到 fake ASR 分段。
5. 停止 Companion ASR service。
6. 确认插件展示 Companion unavailable，而不是回退插件侧 ASR。
...
- v0.2.0 决策：Companion 按 source-only 发布，不发布未签名 `.app` 或 `.dmg`；用户需从源码构建 Companion。
- v0.2.0 决策：Companion 提供 `Load Model` 控制，插件在开始会议前仍会兜底调用 `/model/load` 确保模型 ready。
- 菜单栏常驻模式是否进入 v0.2.0，还是延后到 v0.3.0？
- `companion.json` 写入策略已冻结：状态变化立即写入，`running` 状态下至少每 10 秒刷新一次 `updatedAt`。
- Tauri app 的最低 macOS 版本应定为多少？
- v0.2.0 决策：应用正式名称使用 `EchoNote ASR Companion`。
