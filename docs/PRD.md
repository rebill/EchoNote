...
总结成功后，文件名和一级标题更新为 `YYYY-MM-DD_会议主题`。会议主题由 LLM 返回，日期优先使用会议笔记中的 `Date` 字段。
...
- 长会议音频缓存必须有明确内存上限，超过阈值后转为本地临时文件。
- ASR 模型推理必须串行化，避免不安全的模型并发和内存峰值。
- 长转录总结必须使用有界并发和有界合并输入，并向用户显示进度。
- 说话人分配不得对每个 turn 扫描全部 diarization intervals。
- 发布前必须使用相同输入对比基线，且不得以性能优化为由改变输出顺序或丢失内容。
