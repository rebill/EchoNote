...
- Obsidian 插件通过 discovery 文件优先发现 Companion 管理的 ASR endpoint。
- 插件保留当前手动 Python path / service path / port 配置作为 fallback。
...
- 发言人分离。
- 直接捕获系统音频。
- 移除 v0.1.0 的插件手动 ASR service 管理能力。
...
```ts
type AsrRuntimeMode = "auto" | "companion" | "manual";
...
- 根据 `asrRuntimeMode` 决定 ASR endpoint。
- 在 `auto` 模式下优先 Companion，失败则 manual。
- 在 `companion` 模式下 Companion 不可用即返回错误。
- 在 `manual` 模式下保持 v0.1.0 行为。

返回：

```ts
type AsrRuntime =
  | { mode: "companion"; baseUrl: string; shouldPluginStartService: false }
  | { mode: "manual"; baseUrl: string; shouldPluginStartService: true };
```

### 10.4 Meeting Session 集成

当前 `MeetingSessionController` 直接根据 settings 创建 `AsrServiceClient`，并通过 `AsrProcessManager` 启动服务。

v0.2.0 调整为：

1. `start()` 调用 `resolveAsrRuntime(settings)`。
2. 如果返回 Companion runtime：
   - 跳过 `AsrProcessManager.start()`。
   - 使用 Companion discovery 的 `baseUrl` 创建 `AsrServiceClient`。
3. 如果返回 Manual runtime：
   - 保留现有启动流程。
4. 状态面板记录当前 runtime。
...
```ts
type EchoNoteStatus = {
  asrRuntime: "auto" | "companion" | "manual";
  companionStatus: "unknown" | "available" | "missing" | "stale" | "invalid";
...
sequenceDiagram
  ...
  else Companion unavailable in Auto mode
    P->>P: Fall back to manual ASR runtime
  end
```
...
- discovery missing -> manual fallback。
- discovery stale -> manual fallback。
- discovery invalid -> clear error in Companion mode。
...
- Obsidian plugin：`main.js`、`manifest.json`、`README.md`、zip。
- Companion：源码说明或 macOS `.app` / `.dmg`，取决于是否完成签名和打包。
- `CHANGELOG.md` 更新 v0.2.0。
- `versions.json` 更新插件版本映射。

## 19. 迁移与兼容

- 现有 v0.1.0 用户升级插件后，默认 `Auto` mode。
- 如果用户未安装 Companion，插件应继续使用当前 manual ASR 配置。
- 如果用户安装 Companion，插件可自动发现并优先使用 Companion service。
- 不删除现有 `pythonPath`、`asrServicePath`、`asrServicePort` 设置。
- README 和 Troubleshooting 需要同时说明 Companion 模式和 Manual 模式。
...
| discovery 文件过期 | 插件连到错误服务 | 插件校验 `updatedAt` 并二次 `/health` |
| Tauri 打包复杂度高 | 发布延期 | v0.2.0 可先发布源码构建说明，二进制发布作为后续增强 |
| Companion 与插件状态不一致 | 用户困惑 | discovery 单向描述服务状态，插件状态面板明确 runtime 来源 |
| ASR 子进程残留 | 端口占用或资源泄漏 | Companion 退出时停止自有子进程；启动时检测同端口服务 |

## 22. 开放技术问题

- Companion 是否在 v0.2.0 主动调用 `/model/load`，还是仅展示 model status，由插件在开始会议前加载？
- Tauri 前端是否采用 React，还是先用原生 TypeScript DOM？
- Companion 是否需要菜单栏常驻，还是只做普通窗口应用？
- 是否需要 lock file 防止启动多个 Companion 实例？
- discovery 文件是否需要记录 Companion app version 和 ASR service version？
